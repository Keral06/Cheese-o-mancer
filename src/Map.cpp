...
bool Map::IsPolvo(int gid) {
    static const std::vector<int> ids = { 882, 18967, 2135, 1633, 1636, 819, 2540,20591, 2945, 3369, 2895, 1576, 1575, 3696, 1688, 1682, 1199, 1191, 1189, 1197, 2866 }; // todos los de polvo
    return std::find(ids.begin(), ids.end(), gid) != ids.end();
}                                                                                                           //1682 en temple lvl3 tmb    2866 queen int
                                                                                                       //20591 20592 botanica
bool Map::IsHierba(int gid) {                                                                       //2945 2946 encreuada
    static const std::vector<int> ids = { 883, 20592, 2946, 3370 }; // todos los de hierba        //justes. 3369, 3370 uno polvo otro hierba
    return std::find(ids.begin(), ids.end(), gid) != ids.end();
}
        ...
        if (objectGroupNode.attribute("name").as_string() == std::string("Doors")) {
            for (pugi::xml_node objectNode = objectGroupNode.child("object"); objectNode != NULL; objectNode = objectNode.next_sibling("object")) {
                ...
                if (entityType == "Door") {
                    ...
                    auto keyLevelProp = tempProperties.GetProperty("keyLevel");
                    int keyLevel = keyLevelProp ? keyLevelProp->valueInt : 1;
                    std::string expectedKeyName = "Key" + std::to_string(keyLevel);
