...
bool Audio::EnsureStreams() {
    ...
    if (!music_stream_) {
        ...
        SDL_SetAudioStreamGain(music_stream_, musicVol);
    ...
    if (sfx_streams_.empty()) {
        for (int i = 0; i < MAX_SFX_CHANNELS; ++i) {
            SDL_AudioStream* stream = SDL_CreateAudioStream(nullptr, &device_spec_);
            if (stream) {
                SDL_SetAudioStreamGain(stream, sfxVol);
                if (SDL_BindAudioStream(device_, stream)) {
                    sfx_streams_.push_back(stream);
                }
                else {
                    LOG("Audio: Failed to bind SFX stream %d: %s", i, SDL_GetError());
                    SDL_DestroyAudioStream(stream);
                }
            }
            else {
                LOG("Audio: Failed to create SFX stream %d: %s", i, SDL_GetError());
            }
        }
    }
...
bool Audio::PlayFx(int id, int repeat) {
    ...
    SDL_AudioStream* targetStream = nullptr;
    ...
    if (!targetStream) {
        static int next_steal_index = 0;

        targetStream = sfx_streams_[next_steal_index];

        SDL_ClearAudioStream(targetStream);

        next_steal_index = (next_steal_index + 1) % sfx_streams_.size();
    }

    if (!SDL_SetAudioStreamFormat(targetStream, &s.spec, &device_spec_)) {
        LOG("Audio: SDL_SetAudioStreamFormat(sfx) failed: %s", SDL_GetError());
        return false;
    }

    for (int i = 0; i <= repeat; ++i) {
        if (!SDL_PutAudioStreamData(targetStream, s.buf, s.len)) {
...
void Audio::SetSFXVol(float volume) {
    ...
    for (SDL_AudioStream* stream : sfx_streams_) {
        if (stream) {
            SDL_SetAudioStreamGain(stream, sfxVol);
        }
    }
